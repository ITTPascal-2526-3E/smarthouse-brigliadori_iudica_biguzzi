...
{
    internal class SetAutomaticTurnOnThermosatCommand
    {
        private readonly IThermostatRepository thermostatRepository;

        public SetAutomaticTurnOnThermosatCommand(IThermostatRepository _thermostatRepository)
        {
        ...
        public void Execute(CurrentTemperature targetTemperature, Guid ID)
        {
            ...
            if (thermostat != null)
            {
                thermostat.SetAutomaticTurnOn(targetTemperature);
            }
            else 
            {
